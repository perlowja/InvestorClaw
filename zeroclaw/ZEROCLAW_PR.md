...
**Skill under development**: [InvestorClaw](https://gitlab.com/argonautsystems/InvestorClaw) — a FINOS CDM 5.x-compliant portfolio analysis skill  
...
**InvestorClaw runs flawlessly on OpenClaw**, the companion agent runtime for which it was originally developed. In the current v2.5.0 surface, `/portfolio ask "<question>"` and `/portfolio refresh` execute correctly, produce verified output, and complete within acceptable latency. The issues documented in this report are **specific to zeroclaw** and represent a divergence from the OpenClaw behavior that the skill was designed against.
...
```
For /portfolio commands, use the shell tool to run:
  /portfolio ask "<question>" → investorclaw ask "<question>"
  /portfolio refresh          → investorclaw refresh
```
...
Skill: InvestorClaw v2.0.0 (FINOS CDM 5.x portfolio analysis)
```
...
- InvestorClaw repository: `https://gitlab.com/argonautsystems/InvestorClaw`
