...
> **Platform**: pi-small (Raspberry Pi 4 2GB, aarch64)  
...
InvestorClaw was designed primarily for OpenClaw on pi-large. The zeroclaw runtime on pi-small behaves differently in several critical ways:

| Feature | OpenClaw (pi-large) | zeroclaw (pi-small) |
|---------|------------------|-------------------|
...
[agent]
max_context_tokens = 131072       # ← REQUIRED (was: 32000)
...
sed -i 's/max_context_tokens = 32000/max_context_tokens = 131072/' ~/.zeroclaw/config.toml
...
```
When asked to run /portfolio commands, use the shell tool:
...
| Path | Purpose |
|------|---------|
| `SKILL.toml` | OpenClaw plugin tool definitions (pi-large) |
| `zeroclaw/SKILL.md` | zeroclaw knowledge document (pi-small) |
...
### Why Not SKILL.toml on pi-small?
...
Fix: `open_skills_enabled = false` + `max_context_tokens = 131072`.
...
- SSH to pi-small: `python3 --version` → Python **3.13.5** (host)
